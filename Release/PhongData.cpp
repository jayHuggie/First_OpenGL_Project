...
void MySetupLights()
{
    ...
    double loop = currentTime;
    myLightSphere.InitializeAttribLocations(vertPos_loc); 
    
    // First light (light #0).
    myLights[0].AmbientColor.Set(0.6, 0.6, 0.6);    // Gray color
    ...
    myLights[1].SpotCosCutoff = 0.9f;
    myLights[1].IsSpotLight = true;
    myLights[1].SpotExponent = 1.0f;
    myLights[1].IsEnabled = true;                   // BE SURE TO ENABLE YOUR LIGHTS


    // Third light (light #2)
    
    double rotationAngle = currentTime * PI;
    //myLights[2].AmbientColor.Set(0.0, 0.0, 0.0);    // Yellow color

    /* for (int i = 0; i < 101; i++) {
        myLights[2].DiffuseColor.Set(0.0, 0.0 + i * 0.01, 0.0);      // Very light gray
    }*/
   // myLights[2].DiffuseColor.Set(0.0, currentTime, 0.0);
...
void LoadAllLights() 
{
    ...
    myLights[1].SetSpotlightDirection(viewMatrix, VectorR3(0.0, -1.0, 0.0));
...
void MySetupMaterials()
{
    ...
    myMaterials[0].EmissiveColor.Set(0.0, 0.0f, 0.0f);   // THIS SHOULD CHANGE TO 0,0,0 SO IT HAS NO EMISSION
    myMaterials[0].AmbientColor.Set(0.7f, 0.7f, 0.7f);
    myMaterials[0].DiffuseColor.Set(0.8f, 0.8f, 0.8f);
    myMaterials[0].SpecularColor.Set(0.9,0.9,0.9);
    myMaterials[0].SpecularExponent = 90.0;
    ...
    myMaterials[1].EmissiveColor.Set(0.3, 0.4f, 0.3f);   // THIS SHOULD CHANGE TO 0,0,0 SO IT HAS NO EMISSION
    myMaterials[1].AmbientColor.Set(0.7f, 0.7f, 0.7f);
    myMaterials[1].DiffuseColor.Set(0.7f, 0.7f, 0.7f);
    myMaterials[1].SpecularColor.Set(0.9, 0.9, 0.9);
    myMaterials[1].SpecularExponent = 40.0;
