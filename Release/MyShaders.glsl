...
bool InFshape2( vec2 pos );	// Function prototype

vec4 applyTextureFunction() {
	vec2 wrappedTexCoords = fract(theTexCoords);	// Wrap s,t to [0,1].
	if ( InFshape(wrappedTexCoords) ) {		
		return vec4( 0.48, 0.82, 0.32, 1 );                // Golden Brown color inside the "F"
	}
	if ( InFshape2(wrappedTexCoords) ) {		
		return vec4( 0.48, 0.82, 0.32, 1 );                // Golden Brown color inside the "F2"
	}
	else {
...
bool InFshape( vec2 pos ) {
	...
	if ( pos.x<=sideMargin+postWidth && pos.y >= 1- verticalMargin ) {
	    return true;
	}
	return (pos.y <= 0.5+0.5*armWidth && pos.y >= 0.5-0.5*armWidth );
}
bool InFshape2( vec2 pos ) {
	float sideMargin = 0.4;		// Left-to-right, F is in [sideMargin, 1-sideMargin]
	float verticalMargin = 0.1;	// Bottom-to-top, F is in [vertMargin, 1-vertMargin]
	float postWidth = 0.2;      // Width of the F's post
	float armWidth = 0.4;       // Width of the F's arms
	if ( pos.x<sideMargin || pos.x>1.0-sideMargin ||
	         pos.y<verticalMargin || pos.y>1.0-verticalMargin ) {
		 return false;
    }
