...
const double Xmax = 15.0;                // Control x dimensions of viewable scene
const double Ymax = 15.0;                // Control y dimensions of viewable scene
const double Zmin = -15.0, Zmax = 15.0;   // Control z dimensions of the viewable scene

// zDistance equals the initial distance from the camera to the z = Zmax plane
const double zDistance = 280.0;              // Make this value larger or smaller to affect field of view.

double ZextraDistance = 0.0;              // Extra distance we have moved to/from the scene
double ZextraDelta = 0.8;                // Pressing HOME/END moves closer/farther by this amount
const double ZextraDistanceMin = -200.0;
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
   ...
   // case '3':
    ...
    case '3':
    {
        ...
        theLight.IsEnabled = !theLight.IsEnabled;   // Toggle whether the light is enabled.
        //theLight.DiffuseColor.Set(0.0, currentTime, 0.0);
        
            printf("%f\n", currentTime);
            //theLight.DiffuseColor.Set(currentTime, 0.0, 0.0);
            LoadAllLights();
...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "Final Project Jae Hyuk Lee", NULL, NULL);
	if (window == NULL) {
